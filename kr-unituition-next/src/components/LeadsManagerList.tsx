...
export default function LeadsManagerList({ initialLeads }: { initialLeads: Lead[] }) {
  const [leads, setLeads] = useState<Lead[]>(initialLeads);
  const [search, setSearch] = useState('');
  const [activeTab, setActiveTab] = useState<'all' | 'new' | 'contacted' | 'processing' | 'closed'>('all');
  ...
  // 2. Client-side CSV Exporter with UTF-8 BOM
  const exportToCSV = () => {
    const headers = ['Họ tên', 'Số điện thoại', 'Email', 'Tỉnh thành', 'Trường đăng ký', 'Ngày đăng ký', 'Trạng thái', 'Lịch sử xem trường', 'Ghi chú nội bộ'];
    const rows = filteredLeads.map(lead => [
      ...
      lead.visited_schools && Array.isArray(lead.visited_schools) 
        ? lead.visited_schools.map((s: any) => `${s.name} (${formatDate(s.timestamp)})`).join(' -> ')
        : 'Không có dữ liệu',
      lead.notes.map(n => `[${n.author} ${formatDate(n.created_at)}]: ${n.text}`).join('; ')
    ]);
    ...
    const now = new Date();
    const dateStr = `${now.getMonth() + 1}_${now.getFullYear()}`;
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex flex-wrap gap-2">
          {(['all', 'new', 'contacted', 'processing', 'closed'] as const).map((tab) => {
            ...
            return (
              <button
                ...
                className={`px-4 py-2 rounded-full text-xs font-bold transition-all flex items-center gap-1.5 cursor-pointer ${
                  isActive
                    ? 'bg-black text-white'
                    : 'bg-[#f6f6f3] hover:bg-[#e5e5e0] text-[#000000]'
                }`}
              >
                <span>
                ...
                <span className={`text-[10px] px-1.5 py-0.5 rounded-full ${
                  isActive ? 'bg-white/20 text-white' : 'bg-[#e5e5e0] text-[#62625b]'
                }`}>
                  {count}
                </span>
              </button>
        ...
        <button
          ...
          className="h-10 px-4 rounded-[16px] bg-[#e60023] hover:bg-[#cc001f] active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed text-white text-xs font-bold flex items-center gap-2 shadow-md shadow-[#e60023]/10 transition-all cursor-pointer w-full md:w-auto justify-center"
        >
          <Download size={14} />
          Xuất file CSV
        </button>
      ...
      <div className="relative">
        ...
        <input
          ...
          placeholder="Tìm kiếm theo họ tên, số điện thoại, email, tỉnh thành hoặc trường quan tâm..."
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-[#fbfbf9] border-b border-[#dadad3]">
                ...
                <th className="px-6 py-4 text-xs font-bold text-[#000000] uppercase tracking-wider">Email</th>
                ...
                <th className="px-6 py-4 text-xs font-bold text-[#000000] uppercase tracking-wider">Ngày đăng ký</th>
            ...
            <tbody className="divide-y divide-[#e5e5e0]">
              {filteredLeads.length === 0 ? (
                <tr>
                  <td colSpan={8} className="px-6 py-12 text-center text-[#91918c] text-sm font-medium">
                    Không tìm thấy đăng ký tư vấn nào khớp với bộ lọc.
                  </td>
                </tr>
              ) : (
                filteredLeads.map((lead) => (
                  <tr key={lead.id} className="hover:bg-[#fbfbf9]/50 transition-colors">
                    ...
                    <td className="px-6 py-4 text-sm text-[#62625b]">{lead.email || '—'}</td>
                    <td className="px-6 py-4 text-sm text-[#33332e]">{lead.city}</td>
                    <td className="px-6 py-4 text-sm text-[#33332e] max-w-[180px] truncate" title={lead.school_name}>
                ...
                <div className="space-y-4 pt-4 border-t border-[#e5e5e0]">
                  ...
                  <form onSubmit={handleAddNote} className="space-y-3">
                    ...
                    <button
                      ...
                      className="px-4 py-2 bg-[#e60023] hover:bg-[#cc001f] text-white text-xs font-bold rounded-[12px] flex items-center gap-1.5 ml-auto cursor-pointer disabled:opacity-50"
                    >
                  ...
                  <div className="space-y-3">
                    {selectedLead.notes.length === 0 ? (
                      <p className="text-xs text-[#91918c] text-center italic">Chưa có ghi chú.</p>
                    ) : (
